...
const allFinalists = data.filter(function(item){
...
function getYears(arr, getFinalsCB) {
    return getFinalsCB(arr).map(item => item.Year)
}
...
function getWinners(arr, getFinalsCB) {
    return getFinalsCB(arr).map(item => item['Home Team Goals'] > item['Away Team Goals'] ?
...
function getWinnersByYear(arr, getFinalsCB, getYearsCB, getWinnersCB) {
    ...
    return winners.map((item, idx) => `In ${years[idx]}, ${winners[idx]} won the world cup!`)
}
